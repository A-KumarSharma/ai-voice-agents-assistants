...
class AssistantFnc(llm.FunctionContext):
    ...
    def set_temperature(self,
                        zone:Annotated[Zone,llm.TypeInfo(description="the specific")],
                        temp:Annotated[int,llm.TypeInfo(description="the specific")],
                    ):
                        logger.info("set temo - zone %s, temp:%s",zone,temp)
                        self._temperature[Zone(zone)]=temp
                        return f"the trmperature in the {zone}  is now {temp}C"
